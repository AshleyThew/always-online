...
public class MongoDatabase implements Database {
    ...
    public MongoDatabase(NativeExecutor nativeExecutor, String host, int port, String databaseName, String username,
    ...
    private void connect() {
        ...
        try {
            ...
            if (username != null && !username.isEmpty() && password != null && !password.isEmpty()) {
                uri = String.format("mongodb://%s:%s@%s:%d/%s%s", username, password, host, port, databaseName,
                        connectionString != null ? connectionString : "");
            } else {
                uri = String.format("mongodb://%s:%d/%s%s", host, port, databaseName,
                        connectionString != null ? connectionString : "");
            }
    ...
    public void updatePlayer(String username, String ip, UUID uuid) {
        ...
        if (this.collection != null) {
            try {
                ...
                Document update = new Document("name", username)
                        .append("ip", ip)
    ...
    public void save() throws Exception {
        if (this.collection != null) {
            ...
            for (Map.Entry<String, PlayerData> entry : this.cache.entrySet()) {
                ...
                Document filter = new Document("name", username);
                Document update = new Document("name", username)
                        .append("ip", playerData.ipAddress)
                        .append("uuid", playerData.uuid.toString());
