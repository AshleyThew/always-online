...
public class BungeeCommand extends Command {
		...
		if (args.length <= 0) {
			...
			switch (args[0].toLowerCase()) {
				case "toggle":
					...
					sender.sendMessage(TextComponent.fromLegacyText(ChatColor.GOLD + "Mojang offline mode is now "
				...
				case "reload":
					...
					sender.sendMessage(
							TextComponent.fromLegacyText(ChatColor.GOLD + "AlwaysOnline has been reloaded!"));
					break;
				case "debug":
					...
					if (args.length >= 3 && args.length <= 4) {
						...
						if (args.length == 4) {
							...
							if (UUIDUtil.isValidUUID(uuidString)) {
								...
								sender.sendMessage(TextComponent.fromLegacyText(
										ChatColor.GOLD + "AlwaysOnline " + uuidString + " is not a valid UUID"));
								return;
							}
						} else {
							...
							if (uuid == null) {
								sender.sendMessage(TextComponent.fromLegacyText(
										ChatColor.GOLD + "AlwaysOnline player " + username + " not found in database"));
								return;
							}
						}
						...
						if (!this.ao.alwaysOnline.database.isValidIP(ip)) {
							sender.sendMessage(TextComponent
									.fromLegacyText(ChatColor.GOLD + "AlwaysOnline " + ip + " is not a valid IP"));
							return;
						}

						this.ao.alwaysOnline.database.updatePlayer(username, ip, uuid);
						sender.sendMessage(TextComponent
								.fromLegacyText(ChatColor.GOLD + "AlwaysOnline updated " + username + " to " + ip));
					} else {
						sender.sendMessage(TextComponent.fromLegacyText(
								ChatColor.GOLD + "Usage: /alwaysonline updateip <username> <ip> [uuid]"));
					}
					break;
				default:
					this.displayHelp(sender);
	...
	private void displayHelp(CommandSender sender) {
		sender.sendMessage(TextComponent.fromLegacyText(ChatColor.GOLD + "" + ChatColor.STRIKETHROUGH + "----------"
				+ ChatColor.GOLD + "[" + ChatColor.DARK_GREEN + "AlwaysOnline " + ChatColor.GRAY
				+ ao.getDescription().getVersion() + "" + ChatColor.GOLD + "]" + ChatColor.GOLD + ""
				+ ChatColor.STRIKETHROUGH + "----------"));
		sender.sendMessage(TextComponent.fromLegacyText(ChatColor.GOLD + "/alwaysonline toggle - "
				+ ChatColor.DARK_GREEN + "Toggles between mojang online mode"));
		sender.sendMessage(TextComponent.fromLegacyText(
				ChatColor.GOLD + "/alwaysonline enable - " + ChatColor.DARK_GREEN + "Enables the plugin"));
		sender.sendMessage(TextComponent.fromLegacyText(
				ChatColor.GOLD + "/alwaysonline disable - " + ChatColor.DARK_GREEN + "Disables the plugin"));
		sender.sendMessage(TextComponent.fromLegacyText(
		...
		sender.sendMessage(
				TextComponent.fromLegacyText(ChatColor.GOLD + "/alwaysonline updateip <username> <ip> [uuid] - "
						+ ChatColor.DARK_GREEN + "Update a users ip in the database"));
		sender.sendMessage(TextComponent
				.fromLegacyText(ChatColor.GOLD + "" + ChatColor.STRIKETHROUGH + "------------------------------"));
	}
